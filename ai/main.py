...
model_path = "./mistral-7b-instruct-v0.2.Q4_K_M.gguf"# Проверьте формат и имя файла модели

# Инициализация модели с использованием llama_cpp
model = Llama(model_path=str(model_path), n_ctx=2048, n_gpu_layers=20)  # n_ctx можно настроить в зависимости от потребности

# Инициализация списка для хранения сообщений
conversation_history = []

# Функция для генерации описания отеля с учетом истории сообщений
def generate_hotel_description(hotel_data: dict, extra_prompt: str, conversation_history: list) -> str:
...
def handle_user_message(hotel_data: dict, extra_prompt: str):
    ...
    while True:
        ...
        user_message = input("Введите сообщение для улучшения описания отеля (или 'exit' для выхода): ")
        ...
        description = generate_hotel_description(hotel_data, extra_prompt, conversation_history)
